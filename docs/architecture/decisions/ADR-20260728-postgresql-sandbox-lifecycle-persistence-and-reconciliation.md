...
12. Reconciler 只通过 Tenant-scoped、有界 Keyset Query 读取 `Starting`、`Stopping`、`Destroying`，页大小严格为 `1..=200`；Service 使用有界后继探测，只在确有后续行时返回 Continuation。取得 Lease 后，它按原 `sandbox_operation_id` 重放对应幂等 Provider 操作并完成状态；无法取得 Lease 时跳过，不使用 Process-local Split-brain Fallback。
