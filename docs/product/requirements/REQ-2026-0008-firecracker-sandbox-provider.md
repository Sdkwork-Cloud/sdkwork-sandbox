...
- 确定受支持的 Firecracker/Jailer、Linux Kernel、RootFS/Guest Agent 兼容矩阵与不可变 Digest/Signature Authority；禁止 `latest` 或运行时未校验下载。
- 提供真实 Linux KVM x86_64 和/或 aarch64 节点测试环境；Windows、macOS、WSL 和无 `/dev/kvm` 环境不能作为 MicroVm Assurance 证据。
- 评审最小特权 Host Isolation Broker、Jailer 目标 UID/GID、`/dev/kvm` 最小权限、cgroup v2、Network Namespace/Tap、Egress Enforcement、Runtime Data Root 与 Workspace Block Device Attachment 的运维所有权。
...
- Kernel、RootFS、Guest Agent 和可选 Initrd 在 Allocate/Start 前校验 Digest 与签名策略。RootFS 默认只读；Workspace、Temp 与 Cache 使用不同、限额明确的 Guest Block Device 或 Ephemeral Layer。
- Authorized Workspace Attachment 由已评审 Adapter/Broker 提供 Provider-private Reference，并映射为 Guest Data Device；不得根据 `sandbox_workspace_id` 推导 Host Path，也不得把宿主机目录直接暴露给 Guest。
- Host 与 Guest Command Control 使用私有、一次性启动身份绑定的 Vsock/等价 Channel；第一版只声明 Authenticated Guest Readiness，不声称硬件或远程 Attestation。Firecracker API Socket 只存在于 Provider-private Runtime Directory，并使用最小文件权限，不能绑定 TCP 或进入 Workspace。
- 每个 Allocation 使用独立 Network Namespace/Tap。默认拒绝 Egress、Cloud Metadata、Host Control Plane 与横向 Tenant 访问；只有实际 nftables/等价 Policy 验证成功后才声明 Network Capability。
- vCPU/Memory 在 Firecracker Machine Config 固定；cgroup v2 执行 CPU、Memory、PID 与 IO Limit。Limit 缺失或无法验证时 Provider 不进入 Ready。
...
本需求只交付候选 Firecracker Provider 与真实 KVM 证据。没有 Node Enrollment、Scheduler/Admission、Secret/KMS、Production Deployment、Monitoring/Alerting、Incident Runbook、Image Supply Chain 与人工安全评审时，不得标记 `accepted` 或宣称 SaaS Commercial Ready。
