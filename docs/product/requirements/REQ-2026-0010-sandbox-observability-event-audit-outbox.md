...
2026-07-30：已形成由 AsyncAPI、Envelope Schema、Event Catalog、Outbox Contract、Audit Record Schema 与 Observability Catalog 组成的候选机器契约包；当前目录包含 33 个 Event Type 与 32 个 canonical Metric，Outbox 原子性/投递边界、审计安全身份、结构化日志、Trace 传播、backpressure、Command Execution/Output/Cleanup（含固定 `sandbox_cleanup_status`）、Resource Limit/Usage、Admission/Placement/Capacity、Node Enrollment/Identity/Attestation/Verified Inventory、PostgreSQL Quota/Capacity Reservation 事件和事实分离均可静态验证。Service Host Bootstrap 进一步固定有界 Telemetry Adapter、Redaction、Drop Accounting 与独立 Exporter 运维降级，且不把 Telemetry 当作 Audit/Outbox Authority；REQ-2026-0022 进一步要求 Standalone Telemetry 默认禁用或明确隐私安全 Opt-in，禁止源码、Prompt、Transcript、Artifact、Raw Path、Credential 与 Provider-private 内容。`node --test tests/contract/sandbox-observability-contract.contract.test.mjs` 通过（10/10），包含 Local Host、Service Host、Runtime Pool、Lifecycle Idempotency、Workspace Transaction、Standalone Data Residency 与文档对齐的完整 Contract Suite 也已通过；精确当前数字记录在 PLAN-2026-0002 Verification Checkpoint。所有新增契约均保持 `draft`/`implementationAuthorized: false`，REQ 继续等待 Owner、Security/Privacy、Database、Operations、PKI/Attestation、Capacity/Commerce、Retention、Release 与跨仓库 Trace Authority 人工评审。
