...
| `SDKWORK_DATABASE_URL=<canonical-test-url> SDKWORK_DATABASE_TEST_POSTGRES_URL=<same-url> cargo test -p sdkwork-intelligence-sandbox-repository-sqlx --test postgres_repository --locked -- --ignored --nocapture` | PASS: 1 live PostgreSQL integration test. Exact pre-pool URL equality closes default-development-database fallthrough without mutating process-global environment or echoing connection values on mismatch. |
