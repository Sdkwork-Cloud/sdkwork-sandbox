...
| Cargo workspace gates | `fmt --check`, offline `check`, offline `test`, and offline all-target Clippy with `-D warnings` PASS; 37 Rust tests pass and the one explicitly external PostgreSQL fixture remains ignored without `SDKWORK_SANDBOX_TEST_DATABASE_URL`. |
