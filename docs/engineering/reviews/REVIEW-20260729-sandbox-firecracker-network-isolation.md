...
| Cargo workspace gates | `cargo fmt --all -- --check`, offline Check/Test and all-target Clippy with `-D warnings` PASS; 37 unit tests passed and the external PostgreSQL test remained explicitly ignored without `SDKWORK_SANDBOX_TEST_DATABASE_URL`. |
