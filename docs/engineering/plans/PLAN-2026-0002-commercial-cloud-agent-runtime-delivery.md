...
| P1 | Live PostgreSQL verification is not yet release-automated. | A disposable PostgreSQL 17 run passes locally and the test URL can no longer fall through to the default development database, but the default workspace suite intentionally skips the external test and no PostgreSQL 16/17 CI matrix archives it. | Current implementation evidence is stronger, but release evidence is not yet repeatable in CI. |
...
5. PostgreSQL 16/17, KMS test authority, PKI/attestation test authority and artifact signing/provenance pipelines are available.
...
| `SDKWORK_DATABASE_TEST_POSTGRES_URL=<canonical-test-url> cargo test -p sdkwork-intelligence-sandbox-repository-sqlx --test postgres_repository --locked -- --ignored --nocapture` | PASS on disposable `postgres:17-alpine`: 1 passed, 0 failed. The command was re-run with `SDKWORK_DATABASE_URL` absent, proving the test-only URL selects the pool instead of the default development database. |
...
3. Provide or approve real Local CI runners, KVM nodes, PostgreSQL/KMS/PKI test environments and artifact signing authority.
