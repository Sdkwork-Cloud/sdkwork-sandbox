...
Current status: candidate only. `REQ-2026-0007` is `draft`, the corresponding ADR is `proposed`, the architecture/security review is `pending-human-review`, and `sandbox-command-contract.json` sets `implementationAuthorized` to `false`. These files do not authorize a Rust public port, Local Provider, Firecracker Provider, host process, shell, PTY, network, Secret injection, API route, SDK, or deployment profile.

Authorities:

- `sandbox-command-execution-request.schema.json`: bounded Provider-neutral logical executable identifier, Argv, logical working directory, protected environment input, identity, fencing, idempotency, and limits.
